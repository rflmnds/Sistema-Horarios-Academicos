<?php
	...
	$result = mysqli_query($con,$sql) or die('Falha ao configurar horário (Buscar informações)');
	
	while($config = mysqli_fetch_array($result)){
		$con_cod = $config['con_cod'];
		for($i = 1;$i <= 7;$i++){
			$sql = "INSERT INTO horario(con_cod, ds_cod) VALUES ($con_cod, $i)";
			mysqli_query($con,$sql) or die('Falha ao configurar horário (Adicionar hora à grade)');
		}
	}
